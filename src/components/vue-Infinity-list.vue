<template>
  <div class="vue-list" @scroll="handleScroll">
    <!-- <van-pull-refresh v-model="refresh" class="refresh"> -->
    <div
      ref="group"
      class="list"
      :style="{
    ...
    >
      ...
      <slot v-for="(item,index) in previewList" :item="item" :index="index" />
      <!--<li class="line-bottom" :style="{height: lineBottomHeight +'px'}"></li>-->
      <van-divider>我可是有底线的哦</van-divider>
    </div>
    <!-- </van-pull-refresh> -->
...
export default {
  ...
  methods: {
    ...
    handleScroll() {
      ...
      let _height = this.$refs.group.offsetHeight;
      ...
      let _from = parseInt(_scrollTop / this.height, 10) - this._above;
      if (_from < 0) {
        _from = 0;
      }
      ...
      (_scrollTop = this.$el.scrollTop),
        (_height = this.$refs.group.offsetHeight),
